...
epsilon = .0002;
   start = 96000000;;
    ...
    network0 = network;
    alpha = .01;
   network = RMSinit(network);
for i=1:(4*10^6)/seqlen
    ...
    [in1,ta1] = byte2input(sequence(start:fin),sequence(start+1:fin+1),nunits,seqlen);
    start = start + seqlen;
    fin = fin + seqlen;
    ...
    serr = serr+ (terr/seqlen);
    
    err = serr/i;
  
    network = computegradient(network,ta1,ones(size(ta1),'single'));
   ...
   if 1-1/i < dec
       ...
       network = RMSprop(network,network0,dec1,alpha,epsilon,seqlen);
      network.last=1;
      network = initpass(network,size(in1,2),size(in1,3));
      network = ForwardPass(network, in1);
...
function network = RMSinit(network)


l=1;
network.hidden(l).iweights.MS = gpuArray(zeros(size(network.hidden(l).iweights.matrix),'single'));
network.hidden(l).weights.MS = gpuArray(zeros(size(network.hidden(l).weights.matrix),'single'));
network.hidden(l).fweights.MS = gpuArray(zeros(size(network.hidden(l).fweights.matrix),'single'));
network.hidden(l).mweights.MS = gpuArray(zeros(size(network.hidden(l).mweights.matrix),'single'));

network.output.weights.MS = gpuArray(zeros(size(network.output.weights.matrix),'single'));
end
function network = RMSprop(network,network0,dec,alpha,epsilon,n)

l=1;
network.hidden(l).iweights.gradient=network.hidden.iweights.gradient/n;
network.hidden(l).weights.gradient=network.hidden.weights.gradient/n;
network.hidden(l).fweights.gradient=network.hidden.fweights.gradient/n;
network.hidden(l).mweights.gradient=network.hidden.mweights.gradient/n;
network.output.weights.gradient=network.output.weights.gradient/n;




network.hidden(l).iweights.MS = network.hidden(l).iweights.MS*dec + network.hidden(l).iweights.gradient.^2*(1-dec);
network.hidden(l).weights.MS = network.hidden(l).weights.MS*dec + network.hidden(l).weights.gradient.^2*(1-dec);
network.hidden(l).mweights.MS = network.hidden(l).mweights.MS*dec + network.hidden(l).mweights.gradient.^2*(1-dec);
network.hidden(l).fweights.MS = network.hidden(l).fweights.MS*dec + network.hidden(l).fweights.gradient.^2*(1-dec);


network.output.weights.MS = network.output.weights.MS*dec + network.output.weights.gradient.^2*(1-dec);




network.hidden(l).iweights.dW = -1*network.hidden(l).iweights.gradient./sqrt(network.hidden(l).iweights.MS);
network.hidden(l).iweights.dW(isnan(network.hidden(l).iweights.dW))=0;

network.hidden(l).weights.dW = -1*network.hidden(l).weights.gradient./sqrt(network.hidden(l).weights.MS);
network.hidden(l).weights.dW(isnan(network.hidden(l).weights.dW))=0;


network.hidden(l).mweights.dW = -1*network.hidden(l).mweights.gradient./sqrt(network.hidden(l).mweights.MS);
network.hidden(l).mweights.dW(isnan(network.hidden(l).mweights.dW))=0;

network.hidden(l).fweights.dW = -1*network.hidden(l).fweights.gradient./sqrt(network.hidden(l).fweights.MS);
network.hidden(l).fweights.dW(isnan(network.hidden(l).fweights.dW))=0;




network.output.weights.dW = -1*network.output.weights.gradient./sqrt(network.output.weights.MS);
network.output.weights.dW(isnan(network.output.weights.dW))=0;




network.hidden(l).iweights.dW  = network.hidden(l).iweights.dW*epsilon;
network.hidden(l).weights.dW  = network.hidden(l).weights.dW*epsilon;
network.hidden(l).fweights.dW  = network.hidden(l).fweights.dW*epsilon;
network.hidden(l).mweights.dW  = network.hidden(l).mweights.dW*epsilon;
network.output.weights.dW  = network.output.weights.dW*epsilon;

network.hidden(l).iweights.dW = network.hidden(l).iweights.dW+alpha*(network0.hidden(l).iweights.matrix-network.hidden(l).iweights.matrix);
network.hidden(l).weights.dW = network.hidden(l).weights.dW+alpha*(network0.hidden(l).weights.matrix-network.hidden(l).weights.matrix);
network.hidden(l).fweights.dW = network.hidden(l).fweights.dW+alpha*(network0.hidden(l).fweights.matrix-network.hidden(l).fweights.matrix);
network.hidden(l).mweights.dW = network.hidden(l).mweights.dW+alpha*(network0.hidden(l).mweights.matrix-network.hidden(l).mweights.matrix);
network.output.weights.dW = network.output.weights.dW+alpha*(network0.output.weights.matrix-network.output.weights.matrix);




network.hidden(l).iweights.matrix = network.hidden(l).iweights.matrix + network.hidden(l).iweights.dW;
network.hidden(l).weights.matrix = network.hidden(l).weights.matrix + network.hidden(l).weights.dW;
network.hidden(l).fweights.matrix = network.hidden(l).fweights.matrix + network.hidden(l).fweights.dW;
network.hidden(l).mweights.matrix = network.hidden(l).mweights.matrix + network.hidden(l).mweights.dW;
network.output.weights.matrix = network.output.weights.matrix + network.output.weights.dW;
...
function [lcost,bitacc] = evalCrossEntropy(output,targets,omat)
