...
network.output.weights(l).MS = gpuArray(zeros(size(network.output.weights(l).matrix),'single'));
end
end
...
l=1;
for l=1:length(network.hidden);
network.hidden(l).iweights.gradient=network.hidden(l).iweights.gradient/n;
network.hidden(l).weights.gradient=network.hidden(l).weights.gradient/n;

network.output.weights(l).gradient=network.output.weights(l).gradient/n;
...
network.output.weights(l).MS = network.output.weights(l).MS*dec + network.output.weights(l).gradient.^2*(1-dec);




network.hidden(l).iweights.dW = -1*network.hidden(l).iweights.gradient./(sqrt(network.hidden(l).iweights.MS+.000001));


network.hidden(l).weights.dW = -1*network.hidden(l).weights.gradient./(sqrt(network.hidden(l).weights.MS)+.000001);
...
network.output.weights(l).dW = -1*network.output.weights(l).gradient./(sqrt(network.output.weights(l).MS)+.000001);
...
network.output.weights(l).dW  = network.output.weights(l).dW*epsilon;
...
network.output.weights(l).dW = network.output.weights(l).dW+alpha*(network0.output.weights(l).matrix-network.output.weights(l).matrix);
...
network.output.weights(l).matrix = network.output.weights(l).matrix + network.output.weights(l).dW;
end
end
