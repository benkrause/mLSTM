function mLSTMhutter
%#

%to run on CPU, comment out gpuDevice command and uncomment function gather
%and function gpuArray and end of file
gpuDevice(1)
logfname = 'mLSTMhutter.txt';
...
valstart = 90*10^6;
nunits = max(sequence);
valind = valstart:(valstart+(5*10^5)-1);
[vinputs,vtargets] = byte2input(sequence(valind),sequence(valind+1),nunits,1000);



voind = single(ones(size(vtargets)));
...
nhid = 1900;
network = initnetwork(nunits,nhid,nunits);
...
[verr] = test(network,vinputs,vtargets,voind);
...
MS = zeros(network.nparam,1,'single');
...
for i=1:999999999999
  ...
  if i>1
   ...
   seqlen = 100;
    ...
    [in1,ta1] = byte2input(single(sequence(start:fin)),single(sequence(start+1:fin+1)),nunits,seqlen);
    network.last = 0;
    disp(jit)
    jit = 100;
    ...
    if thresh < .01
        thresh = .01;
          ...
          in= in1(:,j:jit:size(in1,2),:);
          ta= ta1(:,j:jit:size(ta1,2),:);
            ...
            gradient = getGradient(network,in,ta,1)/minibatch;
          ...
          MS = dec1*MS + (1-dec1)*gradient.^2;
         
            dW =  (gradient./(sqrt(MS)+.000001));
            dW(isnan(dW)) = 0;
            norm = sqrt(dW'*dW);
            ...
            network = updateV(network,dW);
            network = initpass(network,size(in,2),size(in,3));
            network = ForwardPass(network, in);
                network.last = 1;
          toc
              ...
              if j==jit;
                  network.last = 0;
                  randind = floor(rand()*(maxtrain-(5*10^5)));
                  randind = randind:(randind+5*10^5 -1);
              [intrain,targtrain] = byte2input(sequence(randind),sequence(randind+1),nunits,1000);
     
                  
            [err] = test(network,intrain,targtrain,ones(size(targtrain),'single'));      
            [verr]= test(network,vinputs,vtargets,voind);
            ...
            display(err);
              ...
              bitchar = err/(size(targtrain,3)*size(targtrain,2));
          
              vbitchar = verr/(size(vtargets,3)*size(vtargets,2));
           
                fprintf(log,'iter: %i  train %f val %f  \n',i,bitchar,vbitchar);
                disp(derr)
           
                    berr = verr;
                    W = weights2vect(getW(network));
                    save(weightsfname,'W');
...
for t=size(network.input.outs.v,3):-1:1;
   ...
   hidden(l).ins.statej = hidden(l).ins.statej +  hidden(l).outs.j.*network.hidden(l).ins.v(network.readind,:,t);
            ...
            if ttemp>0
                ...
                % network.hidden(l).outs.j(:,:,t) =   network.hidden(l).outs.j(:,:,t)+ network.hidden(l).weights(i).matrix'*network.hidden(l).outs.j(:,:,ttemp);
